;;; -*- Lisp -*-
;;; chromadb.lisp - Common Lisp bindings for ChromaDB REST API v2
...
(defvar *chroma-api-version* "/api/v2"
...
(defun chroma-api-path (action &optional collection-param)
  "Constructs the v2 API path for a given ACTION and optional COLLECTION-PARAM (either UUID or name)."
  (let ((base "/tenants/default_tenant/databases/default_database/collections"))
    (ecase action
      (:list base)
      (:create base)
      (:get (format nil "~A/~A" base collection-param))
      (:delete (format nil "~A/~A" base collection-param))
      (:add (format nil "~A/~A/add" base collection-param))
      (:get-records (format nil "~A/~A/get" base collection-param))
      (:query (format nil "~A/~A/query" base collection-param))
      (:delete-records (format nil "~A/~A/delete" base collection-param)))))
...
Each collection is decoded as an association list (alist)."
  (chroma-request :get (chroma-api-path :list)))
...
Returns the collection details alist, or nil if not found or unavailable."
  (chroma-request :get (chroma-api-path :get collection-name)))
...
Returns the created collection details alist, or nil if unavailable."
  (let ((payload `((:name . ,name)
    ...
    (chroma-request :post (chroma-api-path :create) :content (chroma-json-payload payload))))
...
Returns t on success, or nil if unavailable."
  ;; First retrieve details to get the UUID (required for v2 delete)
  (let ((coll (chroma-get-collection collection-name)))
    (if (null coll)
        nil
        (let ((collection-id (cdr (assoc :id coll))))
          (multiple-value-bind (response status)
              (chroma-request :delete (chroma-api-path :delete collection-id))
...
(defun chroma-add (collection-id ids &key embeddings metadatas documents)
  "Adds records to the collection identified by COLLECTION-ID (UUID or Name).
IDS must be a list of strings.
EMBEDDINGS, METADATAS, and DOCUMENTS are optional lists of corresponding values.
If EMBEDDINGS is nil and DOCUMENTS is provided, embeddings will be automatically
generated for each document using string->embedding-vector."
  (let* ((resolved-embeddings
           (or (and embeddings
                    (mapcar (lambda (e) (coerce e 'vector)) embeddings))
               (and documents
                    (mapcar (lambda (doc) (string->embedding-vector doc :model "gemini-embedding-2")) documents))
               (mapcar (lambda (id) (declare (ignore id)) #(0.0)) ids)))
         (payload `((:ids . ,(coerce ids 'vector))
                    (:embeddings . ,(coerce resolved-embeddings 'vector)))))
    (when metadatas
    ...
    (chroma-request :post (chroma-api-path :add collection-id)
                    :content (chroma-json-payload payload))))

(defun chroma-get (collection-id &key ids where where-document limit offset include)
  "Retrieves records from the collection identified by COLLECTION-ID (UUID or Name) by ID or filter.
...
Returns the matching records, or nil if unavailable."
  (let ((payload nil))
    ...
    (chroma-request :post (chroma-api-path :get-records collection-id)
                    :content (chroma-json-payload payload))))

(defun chroma-query (collection-id query-embeddings &key (n-results 10) where where-document include)
  "Queries the collection identified by COLLECTION-ID (UUID or Name) for nearest neighbors of QUERY-EMBEDDINGS.
...
Returns the query results, or nil if unavailable."
  (let ((payload `((:query--embeddings . ,(coerce (mapcar (lambda (v) (coerce v 'vector)) query-embeddings) 'vector))
    ...
    (when include
      (push (cons :include (coerce include 'vector)) payload))
    (chroma-request :post (chroma-api-path :query collection-id)
                    :content (chroma-json-payload payload))))

(defun chroma-delete (collection-id &key ids where where-document)
  "Deletes records from the collection identified by COLLECTION-ID (UUID or Name).
IDS is an optional list of strings.
WHERE is an optional metadata filter alist.
WHERE-DOCUMENT is an optional document content filter alist.
Returns the list of deleted IDs, or nil if unavailable."
  (let ((payload nil))
    ...
    (when where-document
      (push (cons :where--document (json-encodable-value where-document)) payload))
    (chroma-request :post (chroma-api-path :delete-records collection-id)
                    :content (chroma-json-payload payload))))
