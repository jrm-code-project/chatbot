...
(defun instantiate-conversation-from-restored-state (restoration)
  ...
  (let* ((persona-source-name (getf restoration :persona-source-name))
    ...
    (let ((restored-bot (conversation-chatbot restored-conv)))
      (setf (chatbot-checkpoint-name restored-bot)
            (getf restoration :checkpoint-name))
      (setf (chatbot-persona-name restored-bot)
            (getf restoration :persona-name))
      (setf (chatbot-persona-source-name restored-bot)
            persona-source-name)
      (setf (chatbot-backend restored-bot)
            (getf restoration :backend))
      (setf (chatbot-model restored-bot)
            (getf restoration :model))
      (setf (chatbot-system-instruction restored-bot)
            (getf restoration :system-instruction))
      (setf (chatbot-system-instruction-path restored-bot)
            (getf restoration :system-instruction-path))
      (setf (chatbot-system-instruction-storage-kind restored-bot)
            (getf restoration :system-instruction-storage-kind))
      (setf (chatbot-temperature restored-bot)
            (getf restoration :temperature))
      (setf (chatbot-top-p restored-bot)
            (getf restoration :top-p))
      (setf (chatbot-parent-name restored-bot)
            (getf restoration :parent-name))
      (setf (chatbot-depth restored-bot)
            (getf restoration :depth))
      (setf (chatbot-token-budget restored-bot)
            (getf restoration :token-budget))
      (setf (chatbot-spent-tokens restored-bot)
            (getf restoration :spent-tokens))
      (setf (chatbot-content-cache-policy restored-bot)
            (or (getf restoration :content-cache-policy)
                +default-content-cache-policy+))
      (setf (chatbot-content-cache-ttl-seconds restored-bot)
            (or (getf restoration :content-cache-ttl-seconds)
                *default-content-cache-ttl-seconds*))
      (setf (chatbot-content-cache-min-tokens restored-bot)
            (or (getf restoration :content-cache-min-tokens)
                *default-content-cache-min-tokens*))
      (setf (chatbot-google-search-p restored-bot)
            (getf restoration :google-search-p))
      (setf (chatbot-gemini-fallback-to-google-p restored-bot)
            (getf restoration :gemini-fallback-to-google-p))
      (setf (chatbot-web-tools-p restored-bot)
            (getf restoration :web-tools-p))
      (setf (chatbot-code-execution-p restored-bot)
            (getf restoration :code-execution-p))
      (setf (chatbot-include-timestamp-p restored-bot)
            (getf restoration :include-timestamp-p))
      (setf (chatbot-include-model-p restored-bot)
            (getf restoration :include-model-p))
      (setf (chatbot-enable-eval-p restored-bot)
            (getf restoration :enable-eval-p))
      (setf (chatbot-enable-git-tools-p restored-bot)
            (getf restoration :enable-git-tools-p))
      (setf (chatbot-filesystem-tools-p restored-bot)
            (getf restoration :filesystem-tools-p))
      (setf (chatbot-filesystem-root-directory restored-bot)
            (getf restoration :filesystem-root-directory))
      (setf (chatbot-filesystem-allowed-directories restored-bot)
            (getf restoration :filesystem-allowed-directories))
      (setf (chatbot-filesystem-allowlist-path restored-bot)
            (getf restoration :filesystem-allowlist-path))
      (setf (chatbot-filesystem-read-only-p restored-bot)
            (getf restoration :filesystem-read-only-p))
      (setf (chatbot-scoped-directory restored-bot)
            (getf restoration :scoped-directory))
      (setf (chatbot-planner-p restored-bot)
    ...
    (setf (conversation-cached-content-name restored-conv)
          (getf restoration :cached-content-name))
    (setf (conversation-cached-content-key restored-conv)
          (getf restoration :cached-content-key))
    (setf (conversation-cached-content-metadata restored-conv)
