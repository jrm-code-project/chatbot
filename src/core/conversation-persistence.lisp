...
(defun apply-restored-chatbot-state (bot restoration persona-source-name)
  ...
  (setf (chatbot-checkpoint-name bot)
        ...
        (getf restoration :include-model-p) ; Note: check against tsc or tests if this might have typo, let's keep original structure: (chatbot-enable-eval-p bot) (getf restoration :enable-eval-p)
        (chatbot-enable-eval-p bot)
