...
5. **Tests remain tightly coupled to literal payload and JSON structure**
   - **Why this is debt:** Shared payload assertion helpers now cover representative OpenAI, Google, and interaction-payload tests, which reduced some of the noisiest direct `assoc`/shape assertions. Even so, many tests still depend on exact nested payload structure and serialized fragments, so benign internal representation changes can still trigger disproportionate test churn.
   - **Evidence:** `tests/tests.lisp:24-59`, `tests/tests-openai.lisp:16-85`, `tests/tests-google.lisp:6-60`, `tests/tests-payloads.lisp:6-78`, `tests/tests-mcp.lisp:43-73`
