...
11. **Tool Execution Layering Decoupling (Resolved - July 2026)**
    * *The Debt*: Core tool dispatcher (`execute-chatbot-tool`) was tightly coupled with knowledge graph side effects (`sync-knowledge-graph-observations`), violating domain layering and impeding isolation testing.
    * *The Fix*: Established a modular, event-driven post-execution hook framework (`*tool-after-execution-hooks*`) and extracted the database synchronization logic into a registered observer, keeping the central dispatcher pure.
