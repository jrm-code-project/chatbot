...
| --- | --- | --- | --- | --- | --- |
| **P0** | Runtime context and configuration | **Legacy ambient globals still coexist with the newer runtime-context model.** | `src/core/vars.lisp` still carries deprecated compatibility globals and legacy-seam fallback rules. The bridge is narrower now — `*default-conversation*` has a dedicated compatibility path, the old generic function-seam mirroring layer is gone, and only the default-context approval callbacks still deliberately mirror active legacy specials — but no-context entry points and compatibility aliases still keep ambient state in play. | This keeps behavior dependent on ambient mutable state and compatibility mirroring, which makes bugs non-local and complicates concurrent or nested execution. The main loop is cleaner than before, but the compatibility layer is still central. | Finish the runtime-context migration: stop adding new legacy-global readers, convert remaining no-context entry points to explicit runtime contexts, then isolate or remove the deprecated aliases and default-context approval compatibility behind one compatibility module. |
...
1. **Pure chat loop migration**
   ...
   - The old generic legacy-global mirroring layer in `src/core/vars.lisp` was also reduced to a dedicated `*default-conversation*` compatibility path plus explicit default-context approval-callback compatibility, and `make-runtime-context` now inherits default conversations from the canonical runtime context instead of the deprecated ambient global.
