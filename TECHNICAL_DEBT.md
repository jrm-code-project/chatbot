...
| **P0** | MCP lifecycle | **Subprocess and thread lifecycle management is still fragile, even after recent shutdown hardening.** | `src/mcp/mcp-lifecycle.lisp` now explicitly tracks stderr threads, prefers stream closure plus graceful process termination before escalation, and now also cleans up launched subprocesses when supervision startup fails partway through. `src/mcp/mcp-startup.lisp` is also thinner than before: startup-entry initialization, server extraction, empty-status application, shared startup-bot creation/reuse, and owned-server shutdown decisions now route through focused helpers. Remaining debt is that startup policy and shutdown fallback still sit close to low-level lifecycle mechanics, and forced thread/process termination remains the last-resort path. | This is the highest operational-risk code in the repo: process teardown races, blocked I/O, and partial startup failures are hard to reproduce and easy to break. The new lifecycle tracking lowers immediate shutdown and partial-startup hazards, but startup policy is still coupled to low-level lifecycle mechanics. | Keep splitting lifecycle into smaller layers: command/environment resolution, process launch, reader supervision, startup orchestration, and shutdown policy. Preserve the tracked-thread/graceful-first shutdown path, but isolate the remaining forced-termination fallback and shared-startup policy behind smaller lifecycle boundaries. |
