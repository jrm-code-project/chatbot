...
| --- | --- | --- | --- | --- | --- |
| **P0** | Runtime context and configuration | **Legacy ambient globals still coexist with the newer runtime-context model.** | `src/core/vars.lisp` still carries deprecated compatibility globals and legacy-seam fallback rules. The bridge is narrower now — `*default-conversation*` has a dedicated compatibility path, no-arg function seams now honor an active explicit runtime context instead of ambient specials, the old generic function-seam mirroring layer is gone, and only default-context compatibility still deliberately mirrors legacy specials — but compatibility aliases and default-context fallback behavior still keep ambient state in play. | This keeps behavior dependent on ambient mutable state and compatibility mirroring, which makes bugs non-local and complicates concurrent or nested execution. The main loop is cleaner than before, but the compatibility layer is still central. | Finish the runtime-context migration: stop adding new legacy-global readers, convert remaining compatibility-only no-context/default-context fallback paths to explicit runtime contexts, then isolate or remove the deprecated aliases behind one compatibility module. |
...
1. **Pure chat loop migration**
   ...
   - The old generic legacy-global mirroring layer in `src/core/vars.lisp` was also reduced to a dedicated `*default-conversation*` compatibility path plus explicit default-context compatibility, no-arg function seams now prefer the active explicit runtime context instead of ambient specials, and `make-runtime-context` now inherits default conversations from the canonical runtime context instead of the deprecated ambient global.
