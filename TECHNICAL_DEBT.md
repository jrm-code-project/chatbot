...
| **P0** | MCP lifecycle | **Subprocess and thread lifecycle management is still fragile, even after recent shutdown hardening.** | `src/mcp/mcp-lifecycle.lisp` now explicitly tracks stderr threads, prefers stream closure plus graceful process termination before escalation, and now also cleans up launched subprocesses when supervision startup fails partway through. Startup launch planning and supervision are thinner than before, but startup and shutdown still live in one module and still rely on forced thread/process termination as the fallback path. `src/mcp/mcp-startup.lisp` also still mixes config parsing, startup, error handling, shared startup state, and shutdown. | This is the highest operational-risk code in the repo: process teardown races, blocked I/O, and partial startup failures are hard to reproduce and easy to break. The new lifecycle tracking lowers immediate shutdown and partial-startup hazards, but startup policy is still coupled to low-level lifecycle mechanics. | Split lifecycle into smaller layers: command/environment resolution, process launch, reader supervision, startup orchestration, and shutdown policy. Keep the tracked-thread/graceful-first shutdown path, but isolate the remaining forced-termination fallback behind a smaller lifecycle boundary. |
