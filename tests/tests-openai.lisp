...
data: [DONE]")
      ...
      (let ((res1 (chat "Hi there" :conversation conv)))
        ...
        (let* ((payload (decode-test-json (first captured-payloads)))
...
data: [DONE]")
            ...
            (let* ((payload (decode-test-json captured-payload))
                   (messages (test-json-elements (test-json-value-any payload '(:messages "messages"))))
                   (texts (message-content-texts (first messages)))
                   (attachment-text (second texts)))
              (fiveam:is (= 2 (length texts)))
...
data: [DONE]")
      ...
      (let* ((payload (decode-test-json captured-payload))
            (messages (test-json-elements (test-json-value-any payload '(:messages "messages"))))
            (texts (message-content-texts (first messages)))
            (prompt-text (first texts))
            (lisp-feed-text (second texts))
            (scheme-feed-text (third texts)))
...
data: [DONE]")
      ...
      (let ((res (let ((*openai-api-key* "test-key"))
        ...
        (let* ((second-payload (decode-test-json (second captured-payloads)))
          ...
          (assert-role/content (first second-messages) "system" "Be helpful")
          (assert-role/content (second second-messages) "user" "Run tool")
          (assert-json-field= assistant-tool-msg :role "assistant")
          (fiveam:is (not (null (cdr (assoc :tool--calls assistant-tool-msg)))))
          (assert-role/content tool-msg "tool" "tool result")
          (fiveam:is (= 4 (length stored-history)))
...
data: [DONE]")
             ...
             (let ((res (chat "Read the file" :conversation conv)))
               ...
               (fiveam:is (= 2 (length captured-payloads)))
               (let* ((second-payload (decode-test-json (second captured-payloads)))
                      (messages (test-json-elements (test-json-value-any second-payload '(:messages "messages")))))
...
data: [DONE]")
      ...
      (let* ((payload (decode-test-json captured-payload))
       ...
       (fiveam:is (string= (format nil "What time is it?~%~%=== Dynamic Context ===~%[14:29 26-Jun-2026]")
      ...
      (let ((updated-messages (conversation-messages conv)))
       ...
       (assert-history-sequence (subseq updated-messages 0 3)
                                (list '("user" "Earlier question")
                                      '("assistant" "Earlier answer")
                                      (list "user" (format nil "What time is it?~%~%=== Dynamic Context ===~%[14:29 26-Jun-2026]"))))))))
...
data: [DONE]")
      ...
      (let* ((payload (decode-test-json captured-payload))
       ...
       (fiveam:is (string= "Earlier question" (cdr (assoc :content (first messages)))))
       (fiveam:is (string= "Earlier answer" (cdr (assoc :content (second messages)))))
       (fiveam:is (string= (format nil "What time is it?~%~%=== Dynamic Context ===~%[14:29 26-Jun-2026] [model: gpt-4o]")
                           (cdr (assoc :content (third messages))))))
      (let ((updated-messages (conversation-messages conv)))
       (fiveam:is (= 4 (length updated-messages)))
       (assert-history-sequence (subseq updated-messages 0 3)
                                (list '("user" "Earlier question")
                                      '("assistant" "Earlier answer")
                                      (list "user" (format nil "What time is it?~%~%=== Dynamic Context ===~%[14:29 26-Jun-2026] [model: gpt-4o]"))))))))
...
(fiveam:test test-openai-normalize-message-content
  (let ((plain "plain text")
        (gemini-text-part '(("text" . "part text")))
        (gemini-fn-call '(("functionCall" . (("name" . "someTool")))))
        (gemini-fn-resp '(("functionResponse" . (("result" . "someResult")))))
        (parts-list '((("text" . "part 1")) (("text" . "part 2"))))
        (parts-vector '((("text" . "part 1")) (("text" . "part 2")))))
    (fiveam:is (string= "plain text" (openai-normalize-message-content plain)))
    (fiveam:is (string= "part text" (openai-normalize-message-content gemini-text-part)))
    (fiveam:is (string= "[Tool Call: someTool]" (openai-normalize-message-content gemini-fn-call)))
    (fiveam:is (search "someResult" (openai-normalize-message-content gemini-fn-resp)))
