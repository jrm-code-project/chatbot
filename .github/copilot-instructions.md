...
- Shared helpers are split by concern instead of living in one utility file: `json-utils.lisp` handles JSON/schema/plist helpers, `logging.lisp` handles logging and token summaries, `http-utils.lisp` wraps outbound HTTP, `text-utils.lisp` owns SSE parsing and text formatting, and the attachment pipeline is split across `attachment-paths.lisp`, `attachment-mime.lisp`, and `attachments.lisp`.
- Runtime flow is also split by concern: `attachment-paths.lisp` owns transient file pathname expansion and deduplication, `attachment-mime.lisp` owns MIME/content-typing policy, `attachments.lisp` owns binary reading plus attachment encoding, `prompt-decoration.lisp` owns transient timestamp/model prompt prefixes, `request-history.lisp` owns persona preload plus shared stateless history shaping, `openai-payloads.lisp` owns OpenAI-compatible request translation, `google-payloads.lisp` owns Google `generateContent` translation, `gemini-payloads.lisp` owns Gemini Interactions translation, `payloads.lisp` now holds only shared payload helpers, `personas.lisp` handles persona loading, `conversations.lisp` defines `new-chat` / `new-chat-persona`, each backend has its own file, and `chat.lisp` is the dispatch entry point.
