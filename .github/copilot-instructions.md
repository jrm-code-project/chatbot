...
- `src/mcp/mcp.lisp` owns Model Context Protocol integration. Shared MCP startup is initialized during system load through `chatbot:initialize-startup-chatbot`, and `new-chat` reuses that shared server list when it already exists. `initialize-startup-chatbot` is idempotent, so repeated calls safely reuse the shared startup chatbot instead of relaunching MCP servers.
